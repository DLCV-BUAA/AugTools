...
from augtools.extensions.get_keyboard_model_extension import GetKeyboardModelExtension

class KeyboardTransform(CharTransform):
    """
    ...
    def __init__(self, action='substitute', aug_char_min=2, aug_char_max=10, aug_char_p=0.3,
                 ...
                 dict_of_path=None, always_apply = True, p = 0.5, keyboard_path = "en.json"):
        super().__init__(
    ...
    def _append_extensions(self):
        return [
            GetKeyboardModelExtension(dict_of_path=self.dict_of_path, mode='char', keyboard_path=self.keyboard_path),
        ]
...
if __name__ == '__main__':
    ...
    keyboard_transform = KeyboardTransform()
    tran = keyboard_transform(text=text,force_apply=True,n=3,aug_word_p=0.3)
    print(text)
    print(tran['text'])