...
class GetWordWord2vecModelExtension(Extension):
    def __init__(self, file_path=None, max_num_vector=None, model_name=None, top_k=100, skip_check=True, method='WORD'):
        self.method = method
        self.model_name = 'GoogleNews-vectors-negative300' if model_name is None else model_name
        self.model = Word2vec(file_path, max_num_vector, self.model_name, top_k=top_k, skip_check=skip_check, method=method)
...
#     'GoogleNews-vectors-negative300.bin': 'https://drive.google.com/uc?export=download&id=0B7XkCwpI5KDYNlNUTTlSS21pQmM',
# }
...
class Word2vec(WordEmbeddings):
    def __init__(self, file_path=None, max_num_vector=None, model_name=None, model_dim=None, top_k=100, skip_check=True , method='WORD'):
        ...
        file_path = self._check_file_path(file_path, model_name)
    ...
    def _get_model(self, file_path=None, max_num_vector=None):
        model = KeyedVectors.load_word2vec_format(file_path, binary=True, limit=max_num_vector)
        return model
    
    def _check_file_path(self, file_path=None, model_name=None, model_dim=None):
        dest_dir = file_path
        if file_path is None or not os.path.exists(file_path):
            dest_dir = DownloadUtil.get_default_dest_dir(None, 'word2vec')
            if not os.path.exists(dest_dir):
                dest_dir = DownloadUtil.download_word2vec()
        if model_name is None:
            model_name = 'GoogleNews-vectors-negative300'
        file_path = os.path.join(dest_dir, '.'.join([model_name, 'bin']))
        return file_path
...
if __name__ == "__main__":
    ...
    extension = GetWordWord2vecModelExtension()
    rs = None
    rs = extension(rs)
