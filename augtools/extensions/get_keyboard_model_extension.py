...
class GetKeyboardModelExtension(Extension):
    def __init__(self, dict_of_path=None, mode='CHAR', keyboard_path='en.json'):
        self.mode = mode
        self.keyboard_path = keyboard_path
        self.model = Keyboard(dict_of_path, mode, keyboard_path)
...
class Keyboard(BaseModel):
    ...
    def _get_model(self, dict_of_path=None):
        # Use default
        if not dict_of_path:
            default_path = os.path.join(LibraryUtil.get_res_dir(), 'text', 'char', 'keyboard', self.keyboard_path)
            #print(default_path)
...
if __name__ == "__main__":
    ...
    extension = GetKeyboardModelExtension()
    rs = None
    rs = extension(rs)
    print(rs['model']('9'))